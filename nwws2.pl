...
my $server   = $cfg->val('Main', 'server');
my $port     = $cfg->val('Main', 'port');
my $username = $cfg->val('Main', 'username') . '@' . $cfg->val('Main', 'server');
my $password = $cfg->val('Main', 'password');
my $resource = $cfg->val('Main', 'resource');
...
while(1) {
	...
	#$Connection->{DEBUG}->Init(level=>3, file=>"stdout");
